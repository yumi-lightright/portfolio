...
# CSVファイルを読み込み
input_file = "csv/skills_and_experience.csv"  # 入力ファイルのパスを指定
...
# 入力CSVファイルを開く
with open('csv/skills_and_experience.csv', newline='', encoding='utf-8') as csvfile:
...
# ストップワードの設定
stopwords = set(["の", "と", "して", "は", "ます", "です", "など","たい","する","ある","いる","こと","これ","それ","あれ",
                 "この", "その", "あり", "よう", "ため", "られる", "れる", "ている", "ている","ます","し","する","なる","こと","もの",
                 "いくつか","いくつかの","いく","いける","頂きます","いただきます","いただく","頂く","対し","まで","いう",
                 "できる","から","のみ","なく","ところ","しか","いずれ","経験","技術","用い","プロジェクト","スキル","求める",
                 "サービス","持っ","おり","エンド","以上","利用","持ち","システム","アプリケーション","また","案件","クライアント","must",
                 "環境","エンジニア","内容","プロダクト","向け","アプリ","仕事","事業","条件","使用","言語","大切","顧客",
                 "いただき","でき","実装","関する","会社","支援","企業","知識","知見","担当","ながら","可能","メンバー","使っ",
                 "おけ","いただける","よる","好き","it","活用","行い","自社","当社","領域","不問","高い","社内","もしくは","レベル",
                 "作成","関連","基礎","以下","行える","取り組める","自己","持つ","アップ",
                 ])
...
job_data[["仕事内容"]].to_csv("processed_csv/processed_job_data.csv", index=False, encoding="utf-8")
requisite_data[["必須条件"]].to_csv("processed_csv/processed_requite_data.csv", index=False, encoding="utf-8")
welcome_data[["歓迎条件"]].to_csv("processed_csv/processed_welcome_data.csv", index=False, encoding="utf-8")
character_data[["求める人物像"]].to_csv("processed_csv/processed_character_data.csv", index=False, encoding="utf-8")
